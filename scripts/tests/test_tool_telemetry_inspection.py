...
def test_exposure_evidence_summary_keeps_missing_sessions_unknown() -> None:
    summary = exposure_evidence_summary(
        [
            Session("observed", "codex", ["a"], {"a"}),
            Session("missing", "codex", ["b"]),
        ]
    )

    assert summary["status"] == "incomplete"
    assert summary["sufficient_for_empirical_frontier"] is False
    assert summary["call_bearing_sessions_without_direct_exposure"] == 1
    assert summary["calls_without_direct_exposure"] == 1


def test_definition_cost_completeness_distinguishes_exact_estimated_and_unknown() -> (
    None
):
    stats = {
        "exact": pipeline.ToolStat(
            "exact", definition_tokens=10, definition_cost_source="explicit"
        ),
        "estimated": pipeline.ToolStat(
            "estimated",
            definition_tokens=20,
            definition_cost_source="telemetry:x:chars/4",
        ),
        "unknown": pipeline.ToolStat("unknown"),
    }

    result = pipeline.definition_cost_completeness(stats)

    assert result["status"] == "incomplete"
    assert result["tools_with_exact_cost"] == 1
    assert result["tools_with_estimated_cost"] == 1
    assert result["unknown_cost_tools"] == ["unknown"]


def test_frontier_measurement_is_directional_when_evidence_or_costs_are_incomplete() -> (
    None
):
    sessions = [
        Session("observed", "codex", ["exact"], {"exact"}),
        Session("missing", "codex", ["unknown"]),
    ]
    stats = {
        "exact": pipeline.ToolStat(
            "exact", definition_tokens=10, definition_cost_source="explicit"
    ...
    result = pipeline.frontier_measurement_summary(
        sessions,
        stats,
        {"exact", "unknown"},
        "observed_only",
    ...
    assert result["frontier_kind"] == "empirical"
    assert result["directional_only"] is True
    assert result["exposure_evidence_sufficient"] is False
    assert result["cost_completeness"]["exact_complete"] is False


def test_frontier_measurement_is_not_empirical_for_counterfactual_exposure() -> None:
    result = pipeline.frontier_measurement_summary(
        [Session("one", "codex", ["tool"], {"tool"})],
        {
            "tool": pipeline.ToolStat(
                "tool", definition_tokens=10, definition_cost_source="explicit"
            )
        },
        {"tool"},
        "all_runtime_tools",
    ...
    assert result["frontier_kind"] == "counterfactual"
    assert result["directional_only"] is True
...
def test_unresolved_cost_estimates_are_separate_empirical_quantiles() -> None:
    ...
    estimated_low = unknown.estimated_cost_low
    estimated_mid = unknown.estimated_cost_mid
    estimated_high = unknown.estimated_cost_high
    assert estimated_low is not None
    assert estimated_mid is not None
    assert estimated_high is not None
    assert estimated_low <= estimated_mid
    assert estimated_mid <= estimated_high
...
def test_cost_scenarios_measure_raw_cluster_reduction() -> None:
    ...
    low_baseline = scenarios["low"]["baseline_tokens_per_session"]
    high_baseline = scenarios["high"]["baseline_tokens_per_session"]
    low_proposed = scenarios["low"]["proposed_tokens_per_session"]
    high_proposed = scenarios["high"]["proposed_tokens_per_session"]
    assert low_baseline is not None
    assert high_baseline is not None
    assert low_proposed is not None
    assert high_proposed is not None
    assert low_baseline <= high_baseline
    assert low_proposed <= high_proposed
