...
def test_manifest_rejects_baseline_missing_historical_capabilities() -> None:
    raw = manifest_raw()
    raw["historical_tool_capability_tools"] = ["exec", "missing_tool"]

    with pytest.raises(ValueError, match="does not retain historical capabilities"):
        build_architecture_manifest(raw)
